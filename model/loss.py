...
class DETRLoss(nn.Module):
    ...
    def get_class_loss(self, indices, class_pred, class_tgt, log=False):
        ...
        target_classes_o = torch.cat([t[J] for t, (_, J) in zip(class_tgt, indices)]).to(src_logits.device)
    ...
    def get_box_loss(self, indices, box_pred, box_tgt, num_boxes):
        ...
        target_boxes = torch.cat([t[J] for t, (_, J) in zip(box_tgt, indices)], dim=0).to(src_boxes.device)
        loss_bbox = F.l1_loss(src_boxes, target_boxes, reduction='none')
        losses = {}
        losses['loss_bbox'] = loss_bbox.sum() / src_boxes.shape[0]
