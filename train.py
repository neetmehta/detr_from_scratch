...
from model import DETR, PositionEmbeddingSine, ResnetBackbone, Transformer, HungarianMatcher, DETRLoss
...
class Trainer:
    ...
    def __init__(self, cfg_path):
        ...
        backbone = ResnetBackbone(resnet_model=cfg.model.backbone.resnet_model, return_layers=cfg.model.backbone.return_layers, dcn=cfg.model.backbone.dcn)
        transformer = Transformer(d_model=cfg.model.transformer.d_model, nhead=cfg.model.transformer.nhead, num_encoder_layers=cfg.model.transformer.num_encoder_layers, num_decoder_layers=cfg.model.transformer.num_decoder_layers, dropout=cfg.model.transformer.dropout)
        pos_embd = PositionEmbeddingSine(num_pos_feats=cfg.model.positional_embedding.num_pos_feats, temperature=cfg.model.positional_embedding.temperature)
        self.detr = DETR(backbone=backbone, num_classes=cfg.model.num_classes, num_queries=cfg.model.num_queries, positional_embedding=pos_embd, transformer=transformer).to(device)
        
        # Loss functions
        matcher = HungarianMatcher(cost_class=cfg.loss.matcher_cost_class, cost_bbox=cfg.loss.matcher_cost_bbox, cost_giou=cfg.loss.matcher_cost_giou)
        self.criterion = DETRLoss(matcher=matcher, weight_dict=cfg.loss.weight_dict, eos_coef=cfg.loss.eos_coef)
